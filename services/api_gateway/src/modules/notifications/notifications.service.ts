...
export class NotificationsService {
  ...
  async enqueueNotification(dto: any) {
    ...
    const claimed = await this.redis.claimRequest(`idempotency:${dto.id}`, this.cfg.idempotencyTTL);
    if (!claimed) {
    ...
    if (dto.scheduledAt) {
      ...
      if (!isNaN(when) && when > Date.now()) {
        ...
        this.logger.log(`Notification scheduled for ${dto.scheduledAt} (id=${dto.id})`);
